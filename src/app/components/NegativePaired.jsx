...
const NegativePaired = () => {
  ...
  const handleResultCheck = () => {
    ...
    // Specify the pattern to look for based on negative subtraction result
    const targetPairs = [
      ["0", "1"], ["1", "0"],
      ["0", "2"], ["2", "0"],
      ["0", "3"], ["3", "0"],
    ...
    // Traverse each row to find specific adjacent negative pairs
    for (let i = 0; i < tableCells.length - 1; i++) {
      const currentCell = tableCells[i];
      const nextCell = tableCells[i + 1];
      const currentNum = parseInt(currentCell.innerText.trim());
      const nextNum = parseInt(nextCell.innerText.trim());

      if (isNaN(currentNum) || isNaN(nextNum)) continue;

      // Calculate negative difference between pairs
      const difference = Math.abs(currentNum - nextNum).toString();

      // Check if the difference matches any target pairs
      targetPairs.forEach(([first, second]) => {
        if (difference === first || difference === second) {
    ...
    // Highlight pairs that appear more than once and update analyticsData
    const highlightedData = [];
    Object.entries(analytics).forEach(([pairKey, cellPairs]) => {
      if (cellPairs.length > 1) {
        ...
        highlightedData.push([pairKey, cellPairs.length]);
      }
    });
